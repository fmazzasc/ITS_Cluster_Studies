...
from ROOT_graph import set_hist_style, set_graph_style, set_legend, fill_hist
...
def multiple_hist(dfs, columns, plot_specifics, filename, logx=False, logy=True, normalized=True, hist_names=None, x_label=None):
    ...
    Parameters
    --------------------------------------------
    - dfs: list of dataframes whose columns will fill the histograms
    ...
    for column in columns:
        ...
        for i, df in enumerate(dfs):
            ...
            if df.dtype == pd.DataFrame:    
                ...
                if normalized:  set_hist_style(hist, f'{column}', 'Normalized counts', line_color=(i+1), y_title_offset=1.7)
                else:           set_hist_style(hist, f'{column}', 'Counts', line_color=(i+1), y_title_offset=1.7)
            else:                           
                fill_hist(hist, df)
                if normalized:  set_hist_style(hist, x_label, 'Normalized counts', line_color=(i+1), y_title_offset=1.7)  
                else:           set_hist_style(hist, x_label, 'Counts', line_color=(i+1), y_title_offset=1.7)
...
def density_scatter(x, y, filename, canvas, plot_specifics, pad=None, logz=True, logy=False, normalized=True, save=True): 
    ...
    set_hist_style(scatter_plot, x_label, y_label, y_title_offset=1.7)
...
def regression(TrainTestData, ModelParams, HyperParamsRange, model_choice='xgboost', beta_flat=False):
    """
    Parameters:
    ----------------------
